            ...
            <div class="toplayer toplayer-producto" v-if="mostrarToplayer" @click.self="cerrar($event)">
                <div class="toplayer_content card">
                    <div class="acciones-cabecera has-text-right pt-3 pr-3 ">
                        <a  href="#" @click.prevent="cerrar($event,'/ingresar')" class="button is-rounded  button-icono mr-2" v-if="!usuarioCarroCompra()"><i class="fa-solid fa-circle-user"></i> Iniciar Sesión</a>
                            ...
                            <div class="column py-0">
                                ...
                                <div class="columns is-align-items-end	 ">
                                    ...
                                    <div class="pt-0 column has-text-right-tablet has-text-centered-mobile botonera">
                                        <button class="button is-rounded is-7 button-icono mr-2" @click.prevent="cerrar($event)"><b><i class="fa-solid fa-cart-shopping"></i> Continuar Comprando</b></button>
                                        <button class="button is-rounded is-7 button-icono" @click.prevent="cerrar($event,'/carro')"><b><i class="fa-solid fa-cart-shopping"></i> Ir a pagar</b></button>
                                    </div>
                                </div>
                            </div>
                        </div>
...
export default {
    ...
    computed:{
        mensajes(){
            ...
            mensajes[this.storeCarroCompra.carro.datos_toplayer.respuesta.tipo] = this.storeCarroCompra.carro.datos_toplayer.respuesta.mensaje;
            return mensajes;
        },
        mostrarToplayer(){
        ...
        imagen(){
            return (this.producto.hasOwnProperty('images'))?this.producto.images[0].src:this.producto.image;
        },
    },
    methods:{
        ...
        cerrar(event,redireccionar = false){
            ...
            this.storeCarroCompra.actualizarCarro({cantidad:1,mostrar:false,producto:false}, 'datos_toplayer');

            if(redireccionar)
                this.$router.push({ path: redireccionar });
        }
    }
}
