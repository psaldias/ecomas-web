    ...
    <main>
        <div class="acciones" v-if="formato == 'normal' && conStock" >
            <div class="columns is-vcentered is-gapless is-mobile">
                <div class="column  is-flex  cantidad">
                    <button class="button is-small is-rounded50" @click="disminuirCantidad()">-</button>
                    <input type="text" class="input is-rounded is-small is-inline-block" v-model="cantidad" readonly>
                    <button class="button is-small is-rounded50 "  @click="this.aumentarCantidad()">+</button>
                </div>
                <div class="column is-narrow">
                    <button v-if="!cargando" class="button is-rounded is-small button-icono comprar" @click.prevent="agregarProducto()"><b><i class="fa-solid fa-cart-shopping"></i> Comprar</b></button>
        ...
        <div class="acciones" v-else-if="formato == 'horizontal' && conStock" >
            ...
            <div class="column has-text-centered">
                <button v-if="!cargando" class="button is-rounded is-small button-icono comprar" @click.prevent="agregarProducto()">
...
export default {
    ...
    computed: {
        conStock() {
            if ((this.stock == null & this.stockStatus == "instock") || parseInt(this.stock) > 0) {
                return true;
            }
            return false;
        },
        limiteStock() {
            if(this.producto.sold_individually)
                return 1;
    ...
    methods: {
        ...
        aumentarCantidad() {
            ...
            else if (this.cantidad + 1 <= this.limiteStock)
                this.cantidad++;
        },
        async agregarProducto() {
            this.cargando = true;
            await this.agregarProductoCarro(this.idProducto, this.cantidad,this.producto, this.toplayer);
            this.cargando = false;
