...
export default {
  ...
  async mounted() {
    ...
    if (!this.storeCarroCompra.carro.validado) await this.validarCompraNormal();
  ...
  methods: {
    ...
    preLlenarDatos() {
      this.dataFormulario.rut.data = this.storeCarroCompra.carro.data.registro.rut
        ? this.storeCarroCompra.carro.data.registro.rut
        : this.usuarioCarroCompra()
        ? this.storeCarroCompra.usuario.usuario_rut
        : "";
      this.dataFormulario.nombre.data = this.storeCarroCompra.carro.data.registro.nombre
        ? this.storeCarroCompra.carro.data.registro.nombre
        : this.usuarioCarroCompra()
        ? this.storeCarroCompra.usuario.user_first_name
        : "";
      this.dataFormulario.apellido_paterno.data = this.storeCarroCompra.carro.data
        .registro.apellido_paterno
        ? this.storeCarroCompra.carro.data.registro.apellido_paterno
        : this.usuarioCarroCompra()
        ? this.storeCarroCompra.usuario.user_last_name.split(" ")[0]
        : "";
      this.dataFormulario.apellido_materno.data = this.storeCarroCompra.carro.data
        .registro.apellido_materno
        ? this.storeCarroCompra.carro.data.registro.apellido_materno
        : this.usuarioCarroCompra()
        ? this.storeCarroCompra.usuario.user_last_name.split(" ")[1]
        : "";
    ...
    async submitFormulario() {
      ...
      this.dataFormulario.nombre.error =
        this.dataFormulario.nombre.data == "" ? true : false;
      this.dataFormulario.apellido_paterno.error =
        this.dataFormulario.apellido_paterno.data == "" ? true : false;
      this.dataFormulario.apellido_materno.error =
        this.dataFormulario.apellido_materno.data == "" ? true : false;

      /** VALIDACIONES EN CASO DE NO ESTAR CONECTADO */
      if (!this.storeCarroCompra.usuarioCarroCompra) {
        /** VALIDAR CAMPO EMAIL */
        this.dataFormulario.email.error = !helpers.validateEmail(
          this.dataFormulario.email.data
        )
          ? true
          : false;

        /** VALIDAR CAMPO CONFIRMAR EMAIL */
        this.dataFormulario.confirmar_email.error =
          this.dataFormulario.confirmar_email.data != this.dataFormulario.email.data
            ? true
            : false;

        /** VALIDAR CAMPO PASSWORD */
        this.dataFormulario.password.error =
          this.dataFormulario.crearCuenta.data == true &&
          this.dataFormulario.password.data.length < 6
            ? true
            : false;
        /** VALIDAR CONFIRMAR PASSWORD */
        this.dataFormulario.confirmar_password.error =
          this.dataFormulario.crearCuenta.data == true &&
          this.dataFormulario.confirmar_password.data != this.dataFormulario.password.data
            ? true
            : false;
      }
