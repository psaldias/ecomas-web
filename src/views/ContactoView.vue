<template>
    <main class="contacto" v-if="!cargando && data">
        <div class="wrapper">
                    ...
                    <form v-on:submit.prevent="enviarFormulario" ref="formularioContacto">
                        <div class="columns is-multiline mt-5">
                            ...
                            <div class="column is-6">
                                <InputBase label="Nombre" placeHolder='ej:Pedro' type="text"
                                    v-model="dataFormulario.nombre" />
                            </div>
                            <div class="column is-6">
                                <InputBase label="Apellido" placeHolder='ej:Perez' type="text"
                                    v-model="dataFormulario.apellido" />
                            </div>

                            <div class="column is-6">
                                <InputBase label="Email" placeHolder='ej:email@ecomas.cl' type="email"
                            ...
                            <div class="column is-6">
                                <div class="field">
                                    ...
                                    <div class="control">
                                        <input class="input" :class="[{ 'is-danger': dataFormulario.telefono.error }]"
                                            placeHolder="ej:+56912341234" type="text"
                                            v-model="dataFormulario.telefono.data" @focus="moverCursor"
                                            @keypress="validarInputTelefono" maxlength="12" />
                                    </div>
                                </div>
                            </div>
                            <div class="column is-12">
                                <InputSelect label="Asunto" placeHolder='Selecciona un asunto' inputClass="is-fullwidth"
                                    :values="valoresAsunto" v-model="dataFormulario.asunto" />
                            </div>
                            <div class="column is-6">
                                <InputSelect label="Comuna" placeHolder='Selecciona una Comuna' :values="valoresComuna"
                                    v-model="dataFormulario.comuna"
                                    :inputClass="{ 'is-fullwidth': true, 'is-loading': cargando_comunas }" />
                            </div>
                            <div class="column is-6">
                                <InputFile label="Cargar Archivo (jpg, png, jpeg, pdf, doc, docx)"
                                    placeHolder='Seleccionar Archivo' type="text" v-model="dataFormulario.archivo"
                                    permitidos="application/msword, application/vnd.ms-excel, application/vnd.ms-powerpoint,text/plain, application/pdf, image/*" />
                            </div>
                            <div class="column is-12">
                                <InputTextarea label="Comentario" placeHolder='Texto...' inputClass="is-fullwidth"
...
export default {
    ...
    data() {
        return {
            ...
            mensaje_exito: '',
            configuracionFormulario: false,
            store_opciones_generales: useOpcionesGeneralesStore(),
        };
    },
    watch: {
        async "store_opciones_generales.cargando"(nuevoValor) {
    ...
    computed: {
        ...
        valoresComuna() {
            if (!this.configuracionFormulario) return [];
