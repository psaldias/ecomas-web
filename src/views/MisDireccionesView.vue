    ...
    <main class="mi-cuenta">
        <div class="wrapper" v-if="usuarioCarroCompra()">
            ...
            <div class="columns is-variable is-4-tablet mt-4">
                ...
                <div class="column">
                    <h2 class="primero"><strong>Bienvenid@!.</strong> Aquí puedes gestionar tu información personal y
                        pedidos.</h2>
                        ...
                        <div class="column">
                            <div class="message  mt-4 is-ecomas">
                                <div class="message-header">
                                    Dirección Despacho
                                    <a href="#" @click.prevent="direccion_despacho.editar = !direccion_despacho.editar"
                                        class="has-text-weight-normal is-size-7"><i
                                            class="fa-regular fa-pen-to-square"></i> Editar</a>
                                </div>
                                <div class="message-body">
                                    <div class="block">
                                        <strong class="primero">Dirección Actual: </strong> <strong>{{
                                            usuarioCarroCompra().shipping.address_1 }}</strong>
                                    </div>
                                    <div class="block">
                                        <strong class="primero">Comentario Dirección: </strong> <strong>{{
                                            usuarioCarroCompra().shipping.comentario_direccion }}</strong>
                                    </div>
                                    <form action="#" @submit.prevent="actualizarDireccionDespacho()"
                                        v-if="direccion_despacho.editar">
                                        <hr>
                                        <Mensajes :mensajes="direccion_despacho.mensajes"></Mensajes>
                                        <div class="field">
                                            <label for="" class="label">Dirección</label>
                                            <GoogleMapsAutocompleteVue v-if="libreriaCargada"
                                                v-on:placechanged="obtenerDireccionDespacho"
                                                :class="{ 'is-danger': direccion_despacho.direccion.error }"
                                                :country="['cl']" id="direccion_google" classname="input input-2  "
                                                placeholder="Ingresar Dirección" key="map1"></GoogleMapsAutocompleteVue>
                                            <div v-if="direccion_despacho.direccion.data.direccionCompleta"
                                                class="is-size-7 mt-2">
                                                Dirección Seleccionada:
                                                <b>{{ direccion_despacho.direccion.data.direccionCompleta }}</b>
                                            </div>
                                        </div>

                                        <div class="field" v-if="libreriaCargada">
                                            <div class="control">
                                                <input class="input input-2"
                                                    placeHolder="Casa Departamento o Condominio " type="text"
                                        ...
                                        <div class="field has-text-right">
                                            <button type="submit" class="button button-2 bg-primero"
                                                v-if="!direccion_despacho.cargando">Actualizar</button>
                                            <CargandoSeccion v-if="direccion_despacho.cargando" class="small">
                                            </CargandoSeccion>
                                        </div>
                                    </form>
                                </div>
                        ...
                        <div class="column">
                            <div class="message  mt-4 is-ecomas ">
                                <div class="message-header">
                                    ...
                                    <a href="#"
                                        @click.prevent="direccion_facturacion.editar = !direccion_facturacion.editar"
                                        class="has-text-weight-normal is-size-7"><i
                                            class="fa-regular fa-pen-to-square"></i> Editar</a>
                                </div>
                                <div class="message-body">
                                    <div class="block">
                                        <strong class="primero">Dirección Actual: </strong> <strong>{{
                                            usuarioCarroCompra().billing.address_1 }}</strong>
                                    </div>
                                    <form action="#" @submit.prevent="actualizarDireccionFacturacion()"
                                        v-if="direccion_facturacion.editar">
                                        <hr>
                                        <Mensajes :mensajes="direccion_facturacion.mensajes" class="mb-4"></Mensajes>
                                        <div class="field">
                                            <label for="" class="label">Dirección</label>
                                            <GoogleMapsAutocompleteVue2 v-if="libreriaCargada"
                                                v-on:placechanged="obtenerDireccionFacturacion"
                                                :class="{ 'is-danger': direccion_facturacion.direccion.error }"
                                                :country="['cl']" id="direccion_google2" classname="input input-2  "
                                                placeholder="Ingresar Dirección" key="map1">
                                            </GoogleMapsAutocompleteVue2>
                                            <div v-if="direccion_facturacion.direccion.data.direccionCompleta"
                                                class="is-size-7 mt-2">
                                                Dirección Seleccionada:
                                                <b>{{ direccion_facturacion.direccion.data.direccionCompleta }}</b>
                                            </div>
                                        </div>

                                        <div class="field has-text-right">
                                            <button type="submit" class="button button-2 bg-primero"
                                                v-if="!direccion_facturacion.cargando">Actualizar</button>
                                            <CargandoSeccion v-if="direccion_facturacion.cargando" class="small">
                                            </CargandoSeccion>
                                        </div>
                                    </form>
                                </div>
