                            ...
                            <div class="block is-size-6">
                                <table class="table is-fullwidth">
                                    <tr v-for="producto in ordenActiva.items" :key="producto.product_id">
                                        <td><b class="primero">{{producto.product_name}}</b> x {{producto.quantity}}</td>
                                        <td class="has-text-right">{{monedaChilena(producto.subtotal)}}</td>
                                    </tr>
                                    <tr>
                                        <td>Subtotal:</td>
                                        <td class="has-text-right">{{monedaChilena(ordenActiva.subtotal)}}</td>
                                    </tr>
                                    <tr v-if="ordenActiva.discount_total != '0'">
                                        <td>Descuento:</td>
                                        <td class="has-text-right">-{{monedaChilena(ordenActiva.discount_total)}}</td>
                                    </tr>
                                    <tr>
                                        <td>Envio:</td>
                                        <td class="has-text-right">{{monedaChilena(ordenActiva.shipping_total)}}</td>
                                    </tr>
                                    <tr>
                                        <td>Método de pago:</td>
                                        <td class="has-text-right">{{ordenActiva.payment_method_title}}</td>
                                    </tr>
                                    <tr class="primero has-text-weight-bold">
                                        <td>Total:</td>
                                        <td class="has-text-right">{{monedaChilena(ordenActiva.total)}}</td>
                                    </tr>
                                </table>
                            </div>
                        </div>
                            ...
                            <div class="block is-size-6">
                                <table class="table is-fullwidth">
                                    <tr>
                                        ...
                                        <td class="has-text-right">{{obtenerDatoMetaData('transactionResponse')}}</td>
                                    </tr>
                                    <tr>
                                        <td>Orden de Compra:</td>
                                        <td class="has-text-right">{{obtenerDatoMetaData('buyOrder')}}</td>
                                    </tr>
                                    <tr>
                                        <td>Código de autorización:</td>
                                        <td class="has-text-right">{{obtenerDatoMetaData('authorizationCode')}}</td>
                                    </tr>
                                    <tr>
                                        <td>Número de tarjeta:</td>
                                        <td class="has-text-right">**** **** **** {{obtenerDatoMetaData('cardNumber')}}</td>
                                    </tr>
                                    <tr>
                                        <td>Tipo de pago:</td>
                                    ...
                                    <tr>
                                        ...
                                        <td class="has-text-right">{{obtenerDatoMetaData('installmentsNumber')}}</td>
                                    </tr>
                                </table>
                            </div>
                        </div>
                ...
                <table class="table is-fullwidth">
                    ...
                    <tbody>
                        <tr v-for="(pedido,index) in pedidosFiltrados" :key="pedido.id">
                            ...
                            <td class="has-text-centered">{{fechaFormateada(pedido.date_created.date)}}</td>
...
export default {
    ...
    computed: {
        ...
        pedidosFiltrados(){
            return (this.pedidos)? this.pedidos.filter(pedido => pedido.status != 'cancelled' ):[];
        }
    },
    methods: {
        cambiarPedidoActivo(pedido){
            if(pedido.status == 'cancelled' || this.order == pedido.id)
                this.order = false;
            else
        ...
        obtenerDatoMetaData(key = false){
            if(key && this.ordenActiva){
                const dato = this.ordenActiva.meta_data.find(meta_data => {  return meta_data.key == key});
                if(dato)
                    return dato.value;
            }
            return '';
