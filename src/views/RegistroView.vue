          ...
          <div class="column">
            <div class="field">
              <label class="checkbox">
                <input v-model="dataFormulario.boletin.data" type="checkbox" />
                Suscribirme al boletín de noticias
              </label>
            </div>
            <div class="field">
              <label class="checkbox">
                <input v-model="dataFormulario.ofertas.data" type="checkbox" />
            ...
            <div class="field">
              ...
              <button
                v-if="!cargando"
                type="submit"
                class="button button-1 bg-primero has-text-white mt-4 is-fullwidth"
...
export default {
  ...
  beforeMount() {
    if (this.usuarioCarroCompra()) this.$router.replace({ path: "/mi-cuenta" });
  },
  mounted() {
    this.dataFormulario.email.data = this.storeUsuario.usuario.email
      ? this.storeUsuario.usuario.email
  ...
  computed: {
    ...
    comunas() {
      ...
      const region = RegionesYComunas.find(
        (region) => region.region_iso_3166_2 == region_seleccionada
      );

      if (region) {
        comunas = region.communes;
        comunas_filtradas = comunas.map((comuna, index) => {
          return { id: comuna, name: comuna };
        });
      }
    ...
    async submitFormulario() {
      ...
      Object.keys(this.dataFormulario).map((key) => {
        ...
        if (this.dataFormulario[key].requerido && this.dataFormulario[key].data == "") {
          ...
          key == "confirmar_email" &&
          (this.dataFormulario[key].data != this.dataFormulario.email.data ||
            !this.validateEmail(this.dataFormulario[key].data))
        ) {
          this.dataFormulario[key].error = true;
          errorGeneral = true;
          ...
          this.dataFormulario[key].data != this.dataFormulario.password.data
        ) {
          this.dataFormulario[key].error = true;
          errorGeneral = true;
        } else if (
          key == "telefono" &&
          !helpers.validarTelefono(this.dataFormulario[key].data.toString())
        ) {
          this.dataFormulario[key].error = true;
          errorGeneral = true;
        } else {
